...
bburl="jdbc:sqlserver://10.0.8.33:1433;databaseName=BBLEARN"
bbclass="com.microsoft.sqlserver.jdbc.SQLServerDriver"
bbuser="sa"
bbpass="Rapier200\$"

bbdb = Sql.newInstance(bburl,bbuser,bbpass)
//bbdb.execute("use BBLEARN")

print "...PROCESSING:"
bb_classrooms = [:]
itssubjlist = []

def lookupBbClassroom(bbcode) {
  qs = "select * from course_main where course_id=$bbcode"
  println "lookupBbClassroom: $qs"
  row = null
  bbdb.eachRow(qs) { r ->
    row = r
  }
...
def checkRow(itscode,bbc,bcode) {
  print "checkRow($itscode,$bbc,$bcode)";
  if(itscode == null || bbc == null) {
    return false
  } else {
    if(bbc instanceof String && bbc.startsWith(itscode) && bcode == '22') {
      println "...row OK!";
      // check if classroom exists already...
      bbinfo = lookupBbClassroom(bbc)
      if(bbinfo != null) {
        println "COURSE ID $bbc is ALREADY EXISTING!!!"
        return false
      }
      return true
    }
...
if(all_lecturers.length > 0) {
  ...
  qs = """select PAANUM,PAASUR,PAAFNME,PAATIL,PAAIDN,GETADR1
    ...
    and PAANUM=PAQNUM and (PAQDTE<SYSDATE and (PAQEDTE is NULL OR PAQEDTE>'01-JAN-2017'))
    and GETADDRTYPE='ET'
